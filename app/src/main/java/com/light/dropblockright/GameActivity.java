...
public class GameActivity extends Activity implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
    ...
    final Shape[] shapes = new Shape[14];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        NUM_ROWS = Integer.parseInt(prefs.getString("num_rows_preference", "11")) + 6;
        NUM_COLUMNS = Integer.parseInt(prefs.getString("num_columns_preference", "19")) + 6;
    ...
    private void ShapesInit() {
        ...
        shapes[10] = new Shape(a, Color.GRAY, BoardCell.BEHAVIOR_IS_FALLING, false);
        a[1][2] = a[2][1] = a[2][2] = a[2][3] = a[3][2] = 0;

        // big cube
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 1;
        shapes[11] = new Shape(a, Color.rgb(117, 101, 57), BoardCell.BEHAVIOR_IS_FALLING, false);
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 0;

        //big H
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 1;
        a[1][2] = a[1][3] = a[4][2] = a[4][3] = 0;
        shapes[12] = new Shape(a, Color.rgb(128, 158, 73), BoardCell.BEHAVIOR_IS_FALLING);
        for (int i = 1; i <= 4; ++i) for (int j = 1; j <= 4; ++j) a[i][j] = 0;

        a[1][2] = a[2][2] = a[2][3] = a[3][2] = 1;
        shapes[13] = new Shape(a, Color.BLACK, BoardCell.BEHAVIOR_IS_FALLING);
        a[1][2] = a[2][2] = a[2][3] = a[3][2] = 0;
    }
    ...
    private void FixGameMatrix() {//void
    ...
    private boolean CreateShape() {//private booolean
        // generate random shape to put on the gameMatrix

        if (difficulty.compareTo("Normal") == 0) {
            currentShape = shapes[random.nextInt(10)];
        } else {
        ...
        currentShape.x = 1 + (NUM_ROWS - 6) / 2;//1 + (NUM_ROWS - 6) / 2;
        currentShape.y = 0;//1
        // put the new generated shape adjacent to the top side of the table if possible
        for (int offset = 0; offset <= 3; ++offset) {
            int i, ii, j, jj;
            boolean ok = true;
            for (ii = currentShape.x, i = 1; i <= 4; ++i, ++ii) {//ここ
                for (jj = currentShape.y + offset, j = 1; j <= 4; ++j, ++jj) {
            ...
            if (ok) {
                for (i = 1, ii = currentShape.x; i <= 4; ++i, ++ii) {//ここ
                    for (j = 1, jj = currentShape.y + offset; j <= 4; ++j, ++jj) {
    ...
    private boolean Check() {
        int k = 0;
        boolean found = false;
        for (int j = NUM_COLUMNS - 4; j >= 3; --j) {//26-4
            boolean ok = true;
            for (int i = 3; i < NUM_ROWS - 3; ++i) {//16-3 13にすると列のすべてを消す
                if (gameMatrix[i][j].getState() == 0) {
                    ok = false;
                }
            }
            if (ok) {
                ...
                for (int i = 3; i < NUM_ROWS - 3; ++i) {
                    ...
                    gameMatrix[i][j + k] = new BoardCell(state, color, behavior);
                }
        ...
        for (int pas = 0; pas < k; ++pas) {
            for (int i = 3; i < NUM_ROWS - 3; ++i) {
                gameMatrix[i][3 + pas] = new BoardCell();
            }
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            boolean moved = MoveShape(RIGHT_DIRECTION, currentShape);
    ...
    void GameInit() {
        ...
        currentShapeAlive = CreateShape();//currentShapeAlive =
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        try {
            ...
            if (inRange(angle, 45, 135)) {
                ...
                if (gamePaused || !currentShapeAlive)
                    return false;
                MoveShape(UP_DIRECTION, currentShape);
                PaintMatrix();
                ...
                if (gamePaused || !currentShapeAlive)
                    return false;
                MoveShape(DOWN_DIRECTION, currentShape);
                PaintMatrix();
            }
